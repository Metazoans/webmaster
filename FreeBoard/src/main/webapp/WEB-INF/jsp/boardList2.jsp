...
<nav aria-label="Page navigation example">
	<ul class="pagination justify-content-center">
		<%if (paging.isPrev()) { %>
			<li class="page-item">
				<a class="page-link" href="boardList.do?searchCondition=<%=sc %>&keyword=<%=kw %>&page=<%=paging.getStartPage()-1 %>">Prev</a>
			</li>
		<%} else {%>
			<li class="page-item disabled">
				<a class="page-link" href="boardList.do?searchCondition=<%=sc %>&keyword=<%=kw %>&page=<%=paging.getStartPage() %>">Prev</a>
			</li>
		<%} %>
		...
		<%} else {%>
			<li class="page-item disabled">
				<a class="page-link" href="boardList.do?searchCondition=<%=sc %>&keyword=<%=kw %>&page=<%=paging.getEndPage() %>">Next</a>
			</li>
		<%} %>
	</ul>
</nav>
